(function () {
    ...
    const EDUCATION_TYPE_OPTIONS = [
        ...
        { value: "graduate", label: "Graduate Education" },
    ...
    const TEMPLATE = `
        <section class="pedu" aria-label="Education editor">
            <style>
                .pedu {
                    display: grid;
                    gap: 1rem;
    ...
    function parseEducationEntriesFromIndividual(individual) {
        const nodes = Array.isArray(individual?.children)
            ? individual.children.filter((node) => node && (node.tag === "EDUC" || node.tag === "GRAD"))
            : [];
    ...
    async function loadEducationFromGedcomXref(xref) {
        ...
        if (!key) return [];
        if (!window.__profileEducationGedcomLookup) {
            window.__profileEducationGedcomLookup = new Map();
        }
        if (window.__profileEducationGedcomLookup.has(key)) {
            return cloneJson(window.__profileEducationGedcomLookup.get(key));
        }
        if (!window.__profileEducationGedcomTextPromise) {
            window.__profileEducationGedcomTextPromise = fetch(resolveSiteUrl("data/export-Forest.ged"), { cache: "force-cache" })
                .then((response) => (response.ok ? response.text() : ""))
                .catch(() => "");
        }
        const content = await window.__profileEducationGedcomTextPromise;
        if (!content) {
            window.__profileEducationGedcomLookup.set(key, []);
            return [];
        }
        ...
        window.__profileEducationGedcomLookup.set(key, cloneJson(entries));
    ...
    function normalizeEducationEntries(values) {
        if (typeof window.PeopleDB?.toEducationData === "function") {
            return window.PeopleDB.toEducationData({ education: Array.isArray(values) ? values : [] });
        }
        return (Array.isArray(values) ? values : []).map((entry, index) => ({
            id: String(entry?.id || `edu-${index + 1}`),
            sourceTag: String(entry?.sourceTag || "EDUC").toUpperCase() === "GRAD" ? "GRAD" : "EDUC",
    ...
    class ProfileEducationEditor extends HTMLElement {
        connectedCallback() {
        ...
        triggerToolbarAction(action, detail = {}) {
            if (action === "add-education") {
                const type = String(detail.eduType || "").trim() || null;
                this.#addEducation({ type });
            }
        }
        ...
        #snapshotState() {
            return JSON.stringify(this.getEducationData());
        }
        ...
        async #loadExisting() {
            this.#setStatus("Loading education…");
            ...
            this.__entries = normalizeEducationEntries(window.PeopleDB.toEducationData?.(this.__record) || this.__record.education || []);
        ...
        #legendText(entry, index) {
            return normalizeText(entry?.place) || normalizeText(entry?.title) || `Education entry ${index + 1}`;
        }
        ...
        #renderEntry(entry, index) {
            const entryId = String(entry.id || `edu-${index + 1}`);
            ...
            return `
                <fieldset class="pie__group" data-entry-id="${escapeHtml(entryId)}">
                    ...
                    <div class="pie__row pie__row--align-top">
                        ...
                        <div class="pie__field">
                            <textarea id="${escapeHtml(noteId)}" class="page-editor__field-textarea" rows="4" data-entry-id="${escapeHtml(entryId)}" data-field="note" placeholder="Optional details about what was studied or awarded">${escapeHtml(entry.note || "")}</textarea>
                        </div>
                    </div>
                    <div class="pie__row">
        ...
        #handleInput(event) {
            ...
            if (field === "sourceTag") {
                entry.sourceTag = String(target.value || "").toUpperCase() === "GRAD" ? "GRAD" : "EDUC";
            } else if (field === "type") {
        ...
        async #buildPublishFiles() {
            ...
            record = window.PeopleDB.applyEducationToRecord(record, this.getEducationData());
    ...
    if (!customElements.get("profile-education-editor")) {
        customElements.define("profile-education-editor", ProfileEducationEditor);
    }
})();