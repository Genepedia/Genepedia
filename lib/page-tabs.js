...
(function () {
  ...
  function renderMaintainerWorkflow(maintainerState) {
    ...
    const requestControl = !isMaintainer
      ...
      : '';
    const inviteControl = canManage
      ? `<form class="page-changes__maintainer-form" data-maintainer-invite-form>
          <label class="screen-reader-text" for="page-changes-maintainer-login">GitHub username</label>
          <input
            id="page-changes-maintainer-login"
    ...
    return `
      <div class="page-changes__maintainer-workflow">
        ${error}
        <div class="page-changes__maintainer-toolbar">
          ${inviteControl}
          ${requestControl}
        </div>
        ${queue}
  ...
  function renderPeopleGroup(title, people, emptyText, label, fullWidth = false, extraContent = '') {
      ...
      <section class="page-changes__people" aria-label="People">
        <div class="page-changes__people-grid">
          ...
          ${renderPeopleGroup(
            ...
            'Maintainer',
            false,
            renderMaintainerWorkflow(maintainerState),
          )}
  ...
  function bindChangesView(main, sourcePathOrPaths, onPendingReviewComplete = () => {}, onMaintainersChanged = () => {}) {
    ...
    main.addEventListener('submit', async (event) => {
      ...
      const login = String(formData.get('github_login') || '').trim();
      if (!/^[a-zA-Z0-9-]{1,39}$/.test(login)) {
        window.alert('Enter a valid GitHub username.');
        return;
      }
