...
(function () {
  ...
  function bindChangesView(main, sourcePathOrPaths) {
  ...
  function renderChangesView(commits, sourcePathOrPaths, repo, query = '') {
  ...
  function mountChangesView(container, sourcePathOrPaths, {
    ...
    if (container.dataset.changesBound !== 'true') {
      bindChangesView(container, paths);
    ...
    container.innerHTML = '<p class="page-changes__status">Loading change history…</p>';

    return loadHistory()
      .then(({ commits, repo, paths: responsePaths }) => {
        const sourcePaths = responsePaths?.length ? responsePaths : paths;
        changesData = { commits, repo, sourcePath: sourcePaths[0], sourcePaths };
        container.innerHTML = renderChangesView(commits, sourcePaths, repo, changesFilter);
      })
      .catch((error) => {
        console.error(error);
