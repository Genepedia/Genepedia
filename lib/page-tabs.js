...
(function () {
  ...
  function resolveFileCommitDiffApiUrl(sourcePath, hash) {
    const apiBase = resolveGitHubApiBase();
    if (!apiBase) {
      return '';
    }
  ...
  async function fetchFileCommitDiff(sourcePath, hash) {
    const url = resolveFileCommitDiffApiUrl(sourcePath, hash);
    if (!url) {
      throw new Error('GitHub API base is not configured.');
    }
  ...
  function bindChangesView(main, sourcePath) {
    ...
    main.addEventListener('toggle', async (event) => {
      ...
      if (!panel || !hash || panel.dataset.loaded === 'true') {
        return;
      }
      ...
      try {
        let diff = diffCache.get(hash);
        if (!diff) {
          diff = await fetchFileCommitDiff(sourcePath, hash);
          diffCache.set(hash, diff);
        }

        await ensureHighlightJs();
        panel.innerHTML = renderDiffPanel(diff, sourcePath);
  ...
  function renderCommitRow(commit, sourcePath, repo) {
    ...
    return `
      <li class="page-changes__item">
        ...
        <details class="page-changes__details" data-commit-hash="${escapeHtml(hash)}">
  ...
  function renderCommitList(commits, sourcePath, repo) {
    return `
      <ol class="page-changes__list">
        ${commits.map((commit) => renderCommitRow(commit, sourcePath, repo)).join('')}
      </ol>
    `;
  }

  function renderChangesResults(commits, sourcePath, repo, query) {
    if (!commits.length) {
    ...
    return renderCommitList(commits, sourcePath, repo);
  }

  function renderChangesView(commits, sourcePath, repo, query = '') {
    ...
    return `
      <section class="page-changes" aria-label="Change history">
        ...
        <div class="page-changes__results">
          ${renderChangesResults(filteredCommits, sourcePath, repo, filter)}
        </div>
      </section>
    `;
  }
  ...
  function bindChangesSearch(main, getChangesData, setFilter) {
    main.addEventListener('input', (event) => {
      ...
      if (results) {
        results.innerHTML = renderChangesResults(filteredCommits, data.sourcePath, data.repo, filter);
      }
  ...
  function initPageTabs() {
    ...
    let changesDiffBound = false;
    let changesSearchBound = false;
    ...
    const render = async () => {
      ...
      try {
        ...
        changesData = { commits, repo, sourcePath };
        main.innerHTML = renderChangesView(commits, sourcePath, repo, changesFilter);
        if (!changesDiffBound) {
          bindChangesView(main, sourcePath);
          changesDiffBound = true;
        }
        if (!changesSearchBound) {
          bindChangesSearch(main, () => changesData, (value) => {
            changesFilter = value;
          });
          changesSearchBound = true;
        }
      } catch (error) {
        console.error(error);
