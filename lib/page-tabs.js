...
(function () {
  ...
  // lives in data/people/v1/ownership/<bucket>/<id>.json and the person record
  // (names, events, media, living flag) in data/people/v1/persons/<bucket>/<id>.json.
  ...
  async function fetchOwnershipConfig(personId) {
    ...
    return fetchJsonIfExists(`data/people/v1/ownership/${personDbBucket(id)}/${id}.json`);
  }
  ...
  async function fetchPersonRecord(personId) {
    ...
    return fetchJsonIfExists(`data/people/v1/persons/${personDbBucket(id)}/${id}.json`);
  }
    ...
    window.__peopleLoginDirectoryPromise = (async () => {
      ...
      try {
        const logins = window.App?.loadOwnershipLoginIndex
          ...
          : await fetchJsonIfExists('data/people/v1/index/ownership-logins.json').then((payload) => payload?.logins || {});
