...
(function () {
  ...
  function emptyAttributes() {
    return {
      ...
      allergies: [],
      handedness: null,
      // Optional year-tagged history of measurements (editor-managed). Each entry
      // is { year, value }; the plain field above holds the current value.
      heightHistory: [],
      weightHistory: [],
      shoeSizeHistory: [],
    };
  ...
  function personLink(id, name) {
    ...
    return `<a href="../${escapeHtml(String(id))}/index.html">${escapeHtml(name)}</a>`;
  }
