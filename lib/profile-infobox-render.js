...
(function () {
	...
	function normalizeCountryName(value) {
		return String(value || "")
			...
			.toLowerCase()
			.replace(/&/g, " and ")
			.replace(/\b(the|republic of|state of|federation of|kingdom of)\b/g, " ")
			.replace(/[^a-z0-9]+/g, " ")
			.trim()
			.replace(/\s+/g, " ");
	}
	...
	function getCountryNameToCode() {
		...
		const add = (name, code) => {
			...
			if (normalized && /^[a-z]{2}$/.test(code)) {
				countryNameToCode.set(normalized, code);
			}
		};
		...
		if (typeof Intl !== "undefined" && typeof Intl.DisplayNames === "function") {
			const languages = ["en", "en-GB", "en-US"];
			for (const language of languages) {
				try {
					const regionNames = new Intl.DisplayNames([language], { type: "region" });
					for (const code of ISO_COUNTRY_CODES) {
						add(regionNames.of(code), code.toLowerCase());
					}
				} catch (error) {
					// Older browsers may not support DisplayNames for every locale.
				}
			}
		}
	...
	function countryCodeFromName(value) {
		...
		const direct = text.match(/^[A-Za-z]{2}$/);
		if (direct) {
			const code = direct[0].toLowerCase();
			return ISO_COUNTRY_CODES.includes(code.toUpperCase()) ? code : "";
		}

		const map = getCountryNameToCode();
		const candidates = [text, ...text.split(",").map((part) => part.trim()).reverse()];
	...
	function countryCodeFromLocation(location, placeText = "") {
		const normalized = normalizeLocationData(location, placeText);
		return countryCodeFromName(normalized.countryCode)
			|| countryCodeFromName(normalized.country)
			|| countryCodeFromName(placeText);
	}

	function flagImageForLocation(location, placeText = "") {
		const code = countryCodeFromLocation(location, placeText);
		if (!code) return "";
		const label = normalizeLocationData(location, placeText).country || placeText || code.toUpperCase();
		return `<img class="gp-location__flag" src="https://flagcdn.com/${code}.svg" alt="${escapeHtml(label)} flag" loading="lazy">`;
	}
	...
	function migrateFromMarkup(identityEl) {
		...
		else if (gender.startsWith("i")) data.gender = "intersex";
	...
	function genderIcon(value) {
		return GENDER_ICONS[value] || GENDER_ICONS.unknown;
	}

	// Wrap a place string so it can be picked up by the location map hover popup
	// (see people-page-profile-table.js). The text is duplicated into data-place
	// because the visible text may be re-flowed; the attribute stays canonical.
	function locationSpan(placeText, locationData = null) {
		const text = String(placeText || "").trim();
		if (!text) return "";
		const safe = escapeHtml(text);
		const flag = flagImageForLocation(locationData, text);
		return `<span class="gp-location" data-place="${safe}"><span class="gp-location__text">${safe}</span>${flag}</span>`;
	}
	...
	function buildFragment(data, familyHtml) {
		...
		// Ensure a profile image is always shown: use the person's photo when
		// available, otherwise fall back to the site's default profile SVG. We
		// attempt to resolve a site-aware path when possible so the fallback also
		// works under `file:` URLs during local development.
		let photoSrc = String(data.photo?.src || "").trim();
		let photoAlt = String(data.photo?.alt || "").trim();
		if (!photoSrc) {
			try {
				if (typeof window !== 'undefined' && window.App && typeof window.App.resolveSiteUrl === 'function') {
					photoSrc = window.App.resolveSiteUrl('assets/default-profile-photo.svg');
				} else if (typeof location !== 'undefined' && location.protocol === 'file:') {
					// From a person page like file:///.../people/15/index.html the default
					// asset lives two levels up at ../../assets/...
					photoSrc = new URL('../../assets/default-profile-photo.svg', location.href).href;
				} else {
					photoSrc = '/assets/default-profile-photo.svg';
				}
			} catch (e) {
				photoSrc = '/assets/default-profile-photo.svg';
			}
		}
		rows.push(`    <table-photo><img src="${escapeHtml(photoSrc)}" alt="${escapeHtml(photoAlt)}"></table-photo>`);
		...
		rows.push(`    <table-gender><span class="gp-gender"><i class="bi ${genderIcon(data.gender)} gp-gender__icon" aria-hidden="true"></i>${escapeHtml(genderLabel(data.gender))}</span></table-gender>`);
		...
		const datePlaceRow = (tag, group, extraLines = []) => {
			...
			if (placeText) lines.push(locationSpan(placeText, group.location));
		...
		if (lastResidence) {
			rows.push(`    <table-residence>${locationSpan(lastResidence, data.lastResidenceLocation)}</table-residence>`);
		}
		...
		if (burialPlace || burialDate) {
			...
			const pieces = [];
			if (burialPlace) pieces.push(locationSpan(burialPlace, data.burial.location));
