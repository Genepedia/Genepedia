...
(function () {
	...
	function locationSpan(placeText) {
		const text = String(placeText || "").trim();
		if (!text) return "";
		const safe = escapeHtml(text);
		return `<span class="gp-location" data-place="${safe}">${safe}</span>`;
	}
	...
	function buildFragment(data, familyHtml) {
		...
		const datePlaceRow = (tag, group, extraLines = []) => {
			...
			if (placeText) lines.push(locationSpan(placeText));
		...
		if (lastResidence) {
			rows.push(`    <table-residence>${locationSpan(lastResidence)}</table-residence>`);
		}
		...
		if (burialPlace || burialDate) {
			...
			const pieces = [];
			if (burialPlace) pieces.push(locationSpan(burialPlace));
